...
fn feed_path_host(
    path_handler: &mut PathHandler,
    host: &String,
    methods: &Option<Vec<String>>,
    policies: &Vec<PolicyHandler>,
) {
    match path_handler.hosts.get_mut(host) {
        ...
        None => {
            if host == "*" {
                match methods {
                    None => {
                        path_handler.action = Some(RequestAction {
                            methods: Vec::new(),
                ...
                let request_action = RequestAction {
...
fn feed_host_handler(
    host_handler: &mut HostnameHandler,
    host: &String,
    path: &String,
    methods: &Option<Vec<String>>,
    policies: &Vec<PolicyHandler>,
        ...
        if host == "*" {
            ...
            match host_handler.hosts.get_mut(host) {
                ...
                None => {
                    ...
                    let specific_host_handler = SpecificHostnameHandler {
                    ...
                    host_handler
                        .hosts
                        .insert(host.clone(), specific_host_handler);
                }
            }
        }
    }
...
fn process_endpoint(
    ...
    endpoint: &Endpoint,
    host_handler: &mut HostnameHandler,
    path_host: &mut HashMap<String, PathHandler>,
) {
        ...
        PathType::Vec(endpoint_paths) => {
            for path in endpoint_paths {
                ...
                match path_host.get_mut(path) {
                    ...
                    None => {
                        let mut path_handler = PathHandler {
                            hosts: HashMap::new(),
                            action: None,
                        };
                        feed_path_host(
...
fn pipeline_to_function(
    ...
    for policy in policies {
        match policy {
            Policies::LogPolicy(log_policy) => actions.push(PolicyHandler::Log {
                policy: log_policy.clone(),
            }),
...
pub fn register_handlers(cf: &Config) -> (HostnameHandler, HashMap<String, PathHandler>) {
    let mut host_handler: HostnameHandler = HostnameHandler {
        hosts: HashMap::new(),
        action: None,
    };
